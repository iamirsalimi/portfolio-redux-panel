        ...
        <>
            <div className="flex items-center justify-between pr-5">
                <button
                    ...
                    type="button"
                    class="inline-flex w-fit top-5 left-5 items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg transition-colors sm:hidden hover:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                ...
                <h1 className="text-slate-700 font-bold text-center text-2xl sm:hidden">Portfolio Panel</h1>
            </div>
            ...
            <aside id="default-sidebar" className={`z-40 w-64 h-screen transition-transform fixed sm:!translate-x-0 ${showPanel ? 'translate-x-0' : '-translate-x-full'}`} aria-label="Sidebar">
                <div className="z-[42] h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800 space-y-5">
                    <h1 className="text-slate-700 font-bold text-center text-2xl hidden sm:block">Portfolio Panel</h1>
                    <ul className="space-y-2 font-medium">
                        {links.map(link => (
                            <li onClick={() => setShowPanel(false)}>
                                <NavLink to={`/${link.title === 'Personal Informations' ? '' : link.title}`} className="flex items-center transition-colors duration-200 p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                                    {link.svg}

                                    <span className="flex-1 ms-3 whitespace-nowrap font-bold text-gray-600">{link.title}</span>
                                </NavLink>
                            </li>
                        ))}
                    </ul>
